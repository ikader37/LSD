...
import models.UtilisateurFacade;
...
public class LoginController extends AbstractBean implements Serializable {
   ...
   // @Inject
   // private UtilisateurController utilisateurBean;// = new UtilisateurController();
    @Inject
...
public void changePasswd(){
    Utilisateur u;
    u=isValidLogin(UtilisateurController.getUserConnecte().getLoginuser(), oldpasswd);
    if(u!=null){
        u.setModifiele(UtilisateurController.date());
        u.setModifiepar(UtilisateurController.getUserConnecte().getLoginuser());
    ...
    public String loging() {
        ...
        if (utilisateur != null) {// && utilisateur.getLoginpasswd().equals(passwd)
            UtilisateurController.setUserConnecte(utilisateur);
            FacesContext context = FacesContext.getCurrentInstance();
            HttpServletRequest request = (HttpServletRequest) context.getExternalContext().getRequest();
            request.getSession().setAttribute("utilisateur", utilisateur);
            //System.err.println("PROFIL :"+utilisateur.getIdprofil().getLibelleprofil());
